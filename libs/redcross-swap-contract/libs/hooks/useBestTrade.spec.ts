...
import { chainId, signer } from '../__mocks__/providers';
...
describe('Use Best Trade', () => {
  ...
  it('should use best trade', async () => {
    const { result, waitForValueToChange } = renderHook(() => {
      ...
      return [
        useBestV3TradeExactIn({
          ...
          pools: pipe(
            pools,
            O.map((pools) => createPoolsFromSubgraph(chainId)(pools))
          ),
          quoter,
        }),
        quoter,
    ...
    await waitForValueToChange(() => result.current[0].quotesResults, {
      timeout: 10000,
    });
